...
abstract class $ListResponseCopyWith<T, $Res> {
  ...
  @useResult
  $Res call({@JsonKey(name: 'list') List<T>? list});
}
...
class _$ListResponseImpl<T> implements _ListResponse<T> {
  const _$ListResponseImpl({@JsonKey(name: 'list') final List<T>? list})
      : _list = list;
...
abstract class _ListResponse<T> implements ListResponse<T> {
  const factory _ListResponse({@JsonKey(name: 'list') final List<T>? list}) =
